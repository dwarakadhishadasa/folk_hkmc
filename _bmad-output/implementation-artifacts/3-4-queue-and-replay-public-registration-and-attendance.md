...
- [ ] Implement `Queue And Replay Public Registration And Attendance` according to the acceptance criteria (AC: 1-5)
...
- Implementation focus: Preserve the service-worker queue contract for public registration and attendance while keeping staff contact creation online-only.
- Dependency/decision gate: Depends on stories 3.1-3.3 API shapes and existing service-worker queue contract.
